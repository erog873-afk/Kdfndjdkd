...
/**
 * Минимальная ставка. Показывается серым в пустом поле; пока ставка меньше — кнопка «Отправить» неактивна.
 */
...
export const binPayouts: Record<RowCount, Record<RiskLevel, number[]>> = {
  8: {
    ...
    [RiskLevel.MEDIUM]: [5.2, 1.6, 1.2, 1, 0.4, 1, 1.2, 1.6, 5.2],
...
/**
 * Сложности игры = число рядов пинов. Ячеек с множителями внизу на 1 больше:
 * Легко 8 рядов → 9 ячеек, Средне 12 → 13, Сложно 16 → 17.
 */
export type Difficulty = 'easy' | 'medium' | 'hard';

export const difficultyOptions: { value: Difficulty; label: string; rows: RowCount }[] = [
  { value: 'easy', label: 'Легко', rows: 8 },
  { value: 'medium', label: 'Средне', rows: 12 },
  { value: 'hard', label: 'Сложно', rows: 16 },
];

export const rowsByDifficulty = Object.fromEntries(
  difficultyOptions.map(({ value, rows }) => [value, rows]),
) as Record<Difficulty, RowCount>;
