...
class PlinkoEngine {
   ...
   */
  dropBall() {
    const ballOffsetRangeX = this.pinDistanceX * 0.4;
    ...
    this.fairByBall[ball.id] = takeNonce();
    ...
    if (isServerMode()) {
      ...
      this.serverBets[ball.id] = serverPlaceBet(amount, this.rowCount, this.riskLevel)
        .then((r) => {
          balance.set(r.balance);
          return r.game_id;
        })
        .catch(async () => {
          // Сервер отклонил ставку (не хватает звёзд / ошибка): убираем шарик и берём реальный баланс
          this.discardBall(ball);
          const real = await fetchServerBalance();
          if (real !== null) balance.set(real);
          return null;
        });
    }
  }

  private discardBall(ball: Matter.Body) {
    Matter.Composite.remove(this.engine.world, ball);
    delete this.fairByBall[ball.id];
    delete this.serverBets[ball.id];
  ...
  /** Сообщить серверу итог игры и обновить баланс его ответом. */
  private async settleOnServer(ballId: number, binIndex: number) {
    const gameId = await this.serverBets[ballId];
    delete this.serverBets[ballId];
    if (!gameId) return;
    try {
      const r = await serverSettle(gameId, binIndex);
      balance.set(r.balance);
    } catch {
      const real = await fetchServerBalance();
      if (real !== null) balance.set(real);
  ...
  private handleBallEnterBin(ball: Matter.Body) {
    ...
    if (binIndex !== -1 && binIndex < this.pinsLastRowXCoords.length - 1) {
      ...
      // Звёзды целые: выплата округляется вниз (26 × 0.6 = 15)
      const payoutValue = Math.floor(betAmount * multiplier);
      const profit = payoutValue - betAmount;
      const gameId = uuidv4();
      const fairSnapshot = this.fairByBall[ball.id];
      ...
      if (isServerMode()) {
        this.settleOnServer(ball.id, binIndex);
      } else {
      ...
      addHistory({
        ...
        fair: fairSnapshot && {
          ...fairSnapshot,
          rulesHash: getRulesHash(this.rowCount, this.riskLevel),
          rows: this.rowCount,
          risk: this.riskLevel,
          binIndex,
        },
